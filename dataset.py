...
class DatasetTemplate(torch_data.Dataset):
    ...
    def prepare_data(self, data_dict):
        ...
        if self.training:
            ...
            """
            compute visibility in evaluation
            """
            points = data_dict['points']
            original_points = np.delete(points, 3, axis=1)  # delete col 4:intensity
            ...
            logodds = mapping.compute_logodds(original_points, sensor_origins, time_stamps, pc_range, min(voxel_size))
            # else:
            #     sensor_origins = np.array([0.0, 0.0, 0.0])
            #     logodds, original_mask, sampled_mask = mapping.compute_logodds_no_timestamp(original_points,
